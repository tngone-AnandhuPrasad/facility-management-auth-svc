...
public class AuthController : ControllerBase
{
    ...
    /// Constructor
    /// </summary>
    public AuthController(IMongoLayer db,
 ...
 // [TNGAuthAttribute($"{ACCOUNTS_USERTYPE_MASTER.MASTER}")]
  public ActionResult<FacilityResponse> AddaUser(CreateUserRequest request){
    var response = CreateUserAccount( request);
    return response;
  }
    ...
    try
    {
      if (String.Equals(request.ConfirmPassword, request.Password))
      {
        ...
        if (dbUser != null)
        {
          ...
          var userPassword = this.hashPassword(request.OldPassword, dbUser.Salt);
          ...
          else
          {
            ...
            response.Message = ERROR_SETTINGS.WRONG_OLD_PASSWORD;
            return response;
          }
        }
        else
...
/// <param name="request"></param>
/// <returns></returns>
    private FacilityResponse CreateUserAccount(CreateUserRequest request)
